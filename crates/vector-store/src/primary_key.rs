...
mod tests {
    ...
    fn roundtrip_all_scalar_types() {
        ...
        let values = vec![
            ...
            CqlValue::Time(CqlTime(43200_000_000_000)),
    ...
    fn iter_yields_all_values() {
        let pk: PrimaryKey =
            vec![CqlValue::Int(1), CqlValue::Int(2), CqlValue::Int(3)].into();
    ...
    fn debug_format() {
        let pk: PrimaryKey = vec![CqlValue::Int(42)].into();
        let dbg = format!("{pk:?}");
        assert!(dbg.contains("PrimaryKey"), "Debug should contain 'PrimaryKey': {dbg}");
        assert!(dbg.contains("Int(42)"), "Debug should contain 'Int(42)': {dbg}");
    }
